    ...
    <>
      <Head>
        ...
        <link
          ...
          key="icon16"
        />
        <link rel="icon" href="/favicon/hifi-logo-black.ico" key="favicon" />
        <link rel="manifest" href="/favicon/site.webmanifest" />

        {/* PAGE TITLE - using normal title attribute*/}
        {/* Is the home page */}
        {router.pathname === "/" ? (
          <title>{`${Config.site_name}.com`}</title>
        ) : (
          ""
        )}
        {/* Is a post page */}
        {router.pathname.indexOf("post") !== -1 ? (
          <title>{`${props.metaTitle} | ${Config.site_name}`}</title>
        ) : (
          ""
        )}
        ...
        {props.post && (
          <>
            <meta
              key="twitter:image"
              name="twitter:image"
              content={`${process.env.baseUrl}${props.post.image}`}
            ></meta>
            <meta
              property="og:image"
              content={`${process.env.baseUrl}${props.post.image}`}
                  ...
                  process.env.baseUrl
                    ...
                    "publisher": {
                      ...
                      "logo": {
                        ...
                        "url": "${Config.url}${
                  process.env.baseUrl
                }https://www.hifibridge.com/img/brand-img.png"
                      },
                      "name": "${props.pauthor ? props.pauthor : Config.author}"
                    },
                    "headline": "${props.title} | ${Config.title}",
                    "image": ["${Config.url}${process.env.baseUrl}${
                  props.post.image
                }"],
        ...
        <meta key="twitter:site" name="twitter:site" content="@hifibridge" />
        <meta
          key="twitter:creator"
        ...
        <link rel="preload" as="image" href="/icons/global.svg" />
        <link rel="preload" as="image" href="/icons/virtual.svg" />
        <link rel="preload" as="image" href="/icons/coins.svg" />
